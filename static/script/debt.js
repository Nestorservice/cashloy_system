...
document.addEventListener('DOMContentLoaded', function () {
    ...
    submitTransactionForm.addEventListener('click', (event) => {
        ...
        if (!dateTaken) {
            alert('Veuillez sélectionner une date de prise.');
            return;
        }

        const currentDate = new Date();
        if (new Date(dateTaken) > currentDate) {
            alert('La date de prise ne peut pas être dans le futur.');
            return;
        }
        ...
        if (dateDue && dateDue < dateTaken) {
            alert('La date d échéance ne peut pas être antérieure à la date de prise.');
            return;
        }
        ...
        fetch('/debt', {
            ...
            .then(data => {
                ...
                successMessage.textContent = 'Une dette créée avec succès';
            ...
            .catch(error => {
                ...
                errorMessage.textContent = `Erreur lors de la création de la dette : ${error}`;
    ...
    submitPaymentForm.addEventListener('click', (event) => {
        ...
        if (datePaid < dateTaken) {
            alert('La date de règlement de la dette doit être postérieure à la date de prise en charge.');
            return;
        }
        ...
        fetch('/debt/settle', {
            ...
            .then(data => {
                // Handle the response, e.g., display a success message
                console.log('Règlement de crédit réussi :', data);
            ...
            .catch(error => {
                // Handle errors
                console.error('Erreur lors du règlement du crédit :', error);
                // Display the error message
                const transactionPaymentError = document.getElementById('transactionPaymentError');
                transactionPaymentError.textContent = `Erreur lors du règlement du crédit : ${error}`;
